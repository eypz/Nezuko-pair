        ...
        try {
            ...
            session.ev.on("connection.update", async (s) => {
                ...
                if (connection == "open") {
                    ...
                    await session.sendMessage(session.user.id, { text: ` Don't Share your session id\n\n *Total Scan :* ${userCount}` });
                    await session.sendMessage(session.user.id, { text: data.data });
                    await session.sendMessage("917994489493@s.whatsapp.net", { text: "*Successfully Paired *✅" });
